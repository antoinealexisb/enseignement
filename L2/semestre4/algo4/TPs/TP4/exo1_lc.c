...
struct element{
	int val; //valeur de la liste 1-2-3-4 ...
	struct element *suivant;
};
typedef struct element element;
typedef struct element* lliste;


//AJOUTER DES ELEMENTS
lliste ajouterElementDeb(lliste liste, int val){
...
lliste ajouterElementFin(lliste liste, int val){
	element *tmp = malloc(sizeof(element));
	(*tmp).val = val;
	(*tmp).suivant = NULL;
	if (liste == NULL)
...
//GENERER TAB
lliste genererTableau(void){
	lliste liste = NULL;
	srand(1664);
	for (int i=0; i<taille; i++)
		liste = ajouterElementFin(liste, rand());
	return liste;
		
}

//CHANGER VAL
void changerVal(lliste a, lliste b){
	element *tmp = malloc(sizeof(element));
	tmp->val = a->val;
	a->val = b->val;
	b->val = tmp->val;
...
//ERREUR 
/*void triSelection(lliste liste){
	element *tmp = malloc(sizeof(element));
