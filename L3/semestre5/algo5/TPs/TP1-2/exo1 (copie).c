...
void afficherComp(test t, int nb_listes, int t_mini){
  ...
  saveMat(t.comp, nb_listes, t_mini, '\t', fichier);
  fclose(fichier);
  generationGnu("total","Comparaisons"); 
  //free(fichier);
...
void generationGnu(char *nom, char *descrip){
  ...
  fprintf(fichier,"replot '%s.dat' using 1:7 title 'Tri par tas' with linespoints",nom);
  fclose(fichier);
...
//DIFFERENTE GENERATION DE LISTE
...
void entasser(int tab[], int ind, int size, int *comp, int *perm){
  //parcours toute la première moitié du tab (profondeur)
  if (ind<size/2)
...
int main(void){
  //int *test = gen_liste(12, 100);
  //aff_liste(test, 12);
  //tri_selection(test, 12);
  //tri_insertion(test,12);
  //tri_bulle(test,12);
  //aff_liste(test, 12);
  //printf("%d\n",est_trie(test, 12));
  int tmin, nblist, nbMax, nb_expres, valeurMax, nbGenlist, continuer;
  nb_expres = 1000;
  tmin=100;
  nblist=10;
  valeurMax=100;
  ...
  //aff_test(t, NBTRIS, nblist);
  //printf("\n\n");
  //afficherCompCSV(t, nblist, tmin);
  //afficherPermPlot(t, nblist, tmin);
  //fileExist(t, nblist, tmin);*/
  //int* (*tab[2])(int taille, int max) = {gen_liste,genListeTrie};
  //aff_liste(tab[1](10,100),10);
  return(0);
}
