'''
BOURDON Antoine-Alexis L2 - 2018
antoine-alexis_bourdon@ens.univ-artois.fr
...
def main():
    ...
    col = int (input("Entrez le nombre de colonnes:"))
    cv = int (input("Entrez le nombre de cellules vivantes:"))
    vie = Jeu(lig,col)
    vie.placeVCelsRandom(cv)
    vie.afficheGrille()
    choix = input("Voulez vous afficher la nouvelle génération ? (Y/N)")
    while (choix!="n" and not vie.estGrilleVide()):
        vie.etapeSuivante()
        vie.afficheGrille()
        choix = input("Voulez vous afficher la nouvelle génération ? (Y/N)")
